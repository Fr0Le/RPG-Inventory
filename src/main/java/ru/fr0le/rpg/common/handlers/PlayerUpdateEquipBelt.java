...
public class PlayerUpdateEquipBelt {
		...
		if(prevEquip == -1) {
			for(Map.Entry<Integer, Integer> entry0 : equip0Map.entrySet()) {
				...
				if(target != null && target.dimension == event.entity.dimension) {
					PacketDispatcher.sendTo(new SyncEquipBeltMessage(entry0.getKey(), entry0.getValue()), (EntityPlayerMP) player);
				}
			}
		}
		
		if(MinecraftServer.getServer().getTickCounter() % 20 == 0) {
			ItemStack itemStack = ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotBelt);
			
			if(itemStack != null) {
				Item equip = itemStack.getItem();
				if (equip == LoadItemArmor.belt) currentEquip = 1;
			}
			
			if(prevEquip != currentEquip) {
				equip0Map.put(player.getEntityId(), currentEquip);
				if(prevEquip + currentEquip >= 0) {
					PacketDispatcher.sendToDimension(new SyncEquipBeltMessage(player.getEntityId(), currentEquip), player.dimension);
				}
			}
