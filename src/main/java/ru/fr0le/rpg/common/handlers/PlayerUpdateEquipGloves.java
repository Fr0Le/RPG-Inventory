...
public class PlayerUpdateEquipGloves {
		...
		if(prevEquip == -1) {
			for(Map.Entry<Integer, Integer> entry0 : equip0Map.entrySet()) {
				...
				if(target != null && target.dimension == event.entity.dimension) {
					PacketDispatcher.sendTo(new SyncEquipGlovesMessage(entry0.getKey(), entry0.getValue()), (EntityPlayerMP) player);
				}
			}
		}
		
		if(MinecraftServer.getServer().getTickCounter() % 20 == 0) {
			ItemStack itemStack = ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotGloves);
			...
			if(prevEquip != currentEquip) {
				...
				if(prevEquip + currentEquip >= 0) {
					PacketDispatcher.sendToDimension(new SyncEquipGlovesMessage(player.getEntityId(), currentEquip), player.dimension);
				}
			}
