...
public class PlayerRenderHandler {
	...
	public void onPlayerRenderTick(RenderPlayerEvent.Specials.Post event) {
		...
		if (ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotShoulders) != null) new RenderShoulders().render(player, event.renderer);
		if (ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotBracers) != null) new RenderBracers().render(player, event.renderer);
		if (ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotGloves) != null) new RenderGloves().render(player, event.renderer);
		if (ExtendedPlayer.get(player).inventory.getStackInSlot(ContainerCustomPlayer.slotBelt) != null) new RenderBelt().render(player, event.renderer);		
	}
